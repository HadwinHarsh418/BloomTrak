...
@Component({
  selector: 'app-requested-shifts',
  templateUrl: './requested-shifts.component.html',
  styleUrls: ['./requested-shifts.component.scss']
})
export class RequestedShiftsComponent implements OnInit {
  ...
  getPrmsnData(){
    this.dataService.getPermissionByAdminRole().subscribe(
      (res:any) => {
        if (!res.error) {
          res.body.map(i=>{
              if(i.permission_name == 'Requested Shifts'){
