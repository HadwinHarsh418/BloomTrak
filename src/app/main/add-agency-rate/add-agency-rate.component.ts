...
export class AddAgencyRateComponent implements OnInit {
  ...
  submitted(){
    ...
    this.dataService.agencyRates(body).subscribe((res: any) => {
      if (!res.error) {
        this.tost.successToastr('Agency Rate Created Successfully')
        this.btnShow = false;
  ...
  getPosition(){
    let community_id= this.currentUser?.user_role == 3 || this.currentUser?.user_role == 8 ? this.comid : this.currentUser?.user_role == 4 ? this.currentUser?.com_id : this.currentUser?.id
    this.currentUser?.id
